...
/// <summary>
/// Handles <see cref="UpdateDurationPriceCommand"/>.
/// Applies non-null patches to a <see cref="DurationPrice"/> row.
/// Verifies the price → duration → program ownership chain before saving.
/// Evicts the guided tree cache after saving.
/// </summary>
public sealed class UpdateDurationPriceCommandHandler : IRequestHandler<UpdateDurationPriceCommand>
{
    ...
    private readonly ILogger<UpdateDurationPriceCommandHandler> _logger;

    /// <summary>Initialises the handler with required services.</summary>
    public UpdateDurationPriceCommandHandler(
        ...
        IUnitOfWork uow,
        IMemoryCache cache,
        ILogger<UpdateDurationPriceCommandHandler> logger)
    {
        _programs  = programs;
        _experts   = experts;
    ...
    /// <summary>Patches the price row after verifying the ownership chain, then evicts the cache.</summary>
    ...
    public async Task Handle(UpdateDurationPriceCommand request, CancellationToken cancellationToken)
    {
        _logger.LogInformation(
            "UpdateDurationPrice: user {UserId} updating price {PriceId}",
        ...
        if (!request.IsAdmin)
        {
            ...
            if (expert.Id != program.ExpertId)
                throw new ForbiddenException("You can only update prices on your own programs.");

            if (program.Status is ProgramStatus.Published or ProgramStatus.Archived)
                throw new DomainException(
                    "Prices can only be modified on DRAFT or PENDING_REVIEW programs.");
        }

        if (request.Amount.HasValue)       price.Amount         = request.Amount.Value;
        if (request.CurrencyCode is not null) price.CurrencyCode = request.CurrencyCode.ToUpperInvariant();
        ...
        _logger.LogInformation("UpdateDurationPrice: price {PriceId} updated", request.PriceId);
    }
}
