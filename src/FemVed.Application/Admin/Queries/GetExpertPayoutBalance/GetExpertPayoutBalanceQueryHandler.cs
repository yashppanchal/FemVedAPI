...
/// <summary>
/// Handles <see cref="GetExpertPayoutBalanceQuery"/>.
/// Computes the full financial summary for one expert:
/// total revenue from their programs, their payout share (commissionRate %),
/// platform profit ((100-commissionRate) %), total already paid, and outstanding balance.
/// All figures are broken down per currency.
/// </summary>
public sealed class GetExpertPayoutBalanceQueryHandler
    : IRequestHandler<GetExpertPayoutBalanceQuery, ExpertPayoutBalanceDto>
{
    ...
    private readonly ILogger<GetExpertPayoutBalanceQueryHandler> _logger;

    /// <summary>Initialises the handler.</summary>
    public GetExpertPayoutBalanceQueryHandler(
        ...
        ILogger<GetExpertPayoutBalanceQueryHandler> logger)
    {
        _experts  = experts;
        _programs = programs;
    ...
    /// <summary>
    /// Returns the full financial summary for the requested expert.
    /// </summary>
    ...
    public async Task<ExpertPayoutBalanceDto> Handle(
        GetExpertPayoutBalanceQuery request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("GetExpertPayoutBalance: computing financial summary for expert {ExpertId}",
            request.ExpertId);
        ...
        _logger.LogInformation(
            "GetExpertPayoutBalance: completed for expert {ExpertId} — {OrderCount} paid orders, {PayoutCount} payouts",
